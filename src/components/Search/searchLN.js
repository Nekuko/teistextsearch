export function searchLN(keys, text, keywords, nameMap, characters = [], caseSensitive = false, exactMatch = false, regex = false, namedActive = false, namedCharacters = []) {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    sentences.forEach(sentence => {
      ...
      sentence.characters.forEach((character) => {
        const { start, end, name, name_variant } = character;
      ...
      if (anyMatch && namedActive && characters.length === 0) {
        ...
        for (let character of namedCharacters) {
          ...
          for (let range of rangesToCheck) {
            if (characterToCheck === range.name_variant.toLowerCase()) {
          ...
          for (let checkCharacter of characterToCheck) {
            for (let range of rangesToCheck) {
              if (checkCharacter === range.name_variant.toLowerCase()) {
                if (range.name_variant.toLowerCase() === 'rose oriana') {
                }
                characterFound = true;
                break;
              }
