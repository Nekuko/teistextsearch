export function search(keys, text, keywords, caseSensitive = false, exactMatch = false) {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      let filteredSentences = sentences.filter(sentence => {
        ...
        for (let keyword of keywords) {
          ...
          if (exactMatch) {
            // If exact match is required, check if the sentence is exactly the same as the keyword
            if (sentenceToCheck !== keywordToCheck) {
